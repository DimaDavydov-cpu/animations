"use strict";
(self["webpackChunk_fiverr_private_penta_storybook"] = self["webpackChunk_fiverr_private_penta_storybook"] || []).push([[2699],{
...
/***/ "../masonry/src/components/Masonry/stories/index.stories.tsx":
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
var ResizeObserver_es = __webpack_require__("../../node_modules/.pnpm/resize-observer-polyfill@1.5.1/node_modules/resize-observer-polyfill/dist/ResizeObserver.es.js");
...
;// ../masonry/src/components/Masonry/utils/parsePx/index.ts
/**
 * @description Parses string with value in pixels to number
 * @param {string} px - value in pixels
 * @returns {number}
 * @example
 * ```ts
 * const px = '12.25px';
 * console.info(parsePx(px)); // 12.25
 * ```
 */
const parsePx = px => Number(px.replace('px', ''));
;// ../masonry/src/components/Masonry/index.tsx
...
const Masonry = _ref => {
  let {
    children,
    columnCount,
    dataTrackTag = 'masonry',
    dataTrackValue
  } = _ref;
  const containerRef = (0,react.useRef)(null);
  const [ready, setReady] = (0,react.useState)(false);
  const [maxHeight, setMaxHeight] = (0,react.useState)(0);
  const [lineBreaksCount, setLineBreaksCount] = (0,react.useState)(columnCount - 1);
  const forEachTile = callback => {
    const container = containerRef.current;
    if (!container) {
      return;
    }
    container.childNodes.forEach(tileNode => {
      if (tileNode.nodeType !== Node.ELEMENT_NODE || tileNode.dataset.class === 'line-break') {
        return;
      }
      callback(tileNode);
    });
  ...
  const handleResize = entries => {
    if (!containerRef.current || !entries || entries.length === 0) {
      return;
    }
    const container = containerRef.current;
    const containerWidth = container.clientWidth;
    const firstTile = container.firstChild;
    const firstTileWidth = firstTile.clientWidth;
    if (containerWidth === 0 || firstTileWidth === 0) {
      return;
    }
    const firstTileComputedStyle = window.getComputedStyle(firstTile);
    const firstTileMarginLeft = parsePx(firstTileComputedStyle.marginLeft);
    const firstTileMarginRight = parsePx(firstTileComputedStyle.marginRight);
    const currentColumnCount = Math.round(containerWidth / (firstTileWidth + firstTileMarginLeft + firstTileMarginRight));
    const columnHeights = new Array(currentColumnCount).fill(0);
    forEachTile(tile => {
      const tileComputedStyle = window.getComputedStyle(tile);
      const tileMarginTop = parsePx(tileComputedStyle.marginTop);
      const tileMarginBottom = parsePx(tileComputedStyle.marginBottom);
      const tileHeight = parsePx(tileComputedStyle.height);
      const tileHeightFinal = tileHeight + tileMarginBottom + tileMarginTop;
      const columnIndex = columnHeights.indexOf(Math.min(...columnHeights));
      columnHeights[columnIndex] += tileHeightFinal;
      tile.style.order = "".concat(columnIndex + 1);
    });
    ...
    (0,react_dom.flushSync)(() => {
      setLineBreaksCount(currentColumnCount - 1);
      setMaxHeight(Math.ceil(Math.max(...columnHeights)));
      setReady(true);
    });
  };
  const resizeObserverRef = (0,react.useRef)();
  if (!resizeObserverRef.current) {
    resizeObserverRef.current = new ResizeObserver_es/* default */.A(handleResize);
  }
  (0,useIsomorphicLayoutEffect/* default */.A)(() => {
    ...
    forEachTile(tile => {
      ...
      tile.style.width = "".concat((100 / columnCount).toFixed(2), "%");
    });
    return () => {
  ...
  // so line breaks at the end of each column prevent it
  const lineBreaks = new Array(lineBreaksCount).fill('').map((_, index) => /*#__PURE__*/react.createElement(Container/* default */.A, {
    as: "span",
    key: index,
...
/* harmony default export */ const components_Masonry = (Masonry);
try {
    // @ts-ignore
    Masonry.displayName = "Masonry";
    // @ts-ignore
    Masonry.__docgenInfo = { "description": "", "displayName": "Masonry", "props": { "children": { "defaultValue": null, "description": "Tile components.", "name": "children", "required": true, "type": { "name": "boolean", "value": [{ "value": "string" }, { "value": "number" }, { "value": "false" }, { "value": "true" }, { "value": "{}" }, { "value": "ReactElement<any, string | JSXElementConstructor<any>>" }, { "value": "Iterable<ReactNode>" }, { "value": "ReactPortal" }] } }, "columnCount": { "defaultValue": null, "description": "Number of columns.", "name": "columnCount", "required": true, "type": { "name": "number" } }, "dataTrackTag": { "defaultValue": { value: "masonry" }, "description": "", "name": "dataTrackTag", "required": false, "type": { "name": "string" } }, "dataTrackValue": { "defaultValue": null, "description": "", "name": "dataTrackValue", "required": false, "type": { "name": "string" } } } };
    // @ts-ignore
    if (typeof STORYBOOK_REACT_CLASSES !== "undefined")
        // @ts-ignore
        STORYBOOK_REACT_CLASSES["../masonry/src/components/Masonry/index.tsx#Masonry"] = { docgenInfo: Masonry.__docgenInfo, name: "Masonry", path: "../masonry/src/components/Masonry/index.tsx#Masonry" };
}
...
const meta = {
  ...
  component: components_Masonry,
  parameters: {
    docs: {
      source: {
        code: "const defaultHeights = [120, 140, 80, 150, 70, 170, 60, 80, 180, 90];\nconst [heights, setHeights] = useState(defaultHeights);\nconst loadMore = () => {\n  setHeights((value) => {\n    const newValues: string[] = [];\n    for (let i = 0; i < 3; i++) {\n      newValues.push(random(50, 200));\n    }\n    return [...value, ...newValues];\n  });\n};\n\nreturn (\n  <StoryStack direction=\"column\">\n    <Masonry columnCount={4}>\n      {heights.map((height, index) => (\n        <Tile key={index} height={height} index={index} />\n      ))}\n    </Masonry>\n    <StoryStack justifyContent=\"center\">\n      <Button onClick={loadMore}>Load More</Button>\n    </StoryStack>\n  </StoryStack>\n);"
      }
    }
  },
  render: args => {
    const defaultHeights = [120, 140, 80, 150, 70, 170, 60, 80, 180, 90];
    const [heights, setHeights] = (0,react.useState)(defaultHeights);
    const loadMore = () => {
      setHeights(value => {
        const newValues = [];
        for (let i = 0; i < 3; i++) {
          newValues.push((0,lodash.random)(50, 200));
        }
        return [...value, ...newValues];
      });
    };
    return /*#__PURE__*/react.createElement(StoryStack/* default */.A, {
      direction: "column"
    }, /*#__PURE__*/react.createElement(components_Masonry, args, heights.map((height, index) => /*#__PURE__*/react.createElement(Tile, {
      key: index,
      height: height,
      index: index
    }))), /*#__PURE__*/react.createElement(StoryStack/* default */.A, {
...
//# sourceMappingURL=2699.61fdcd2b.iframe.bundle.js.map